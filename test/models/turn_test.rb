...
class TurnTest < ActiveSupport::TestCase
  ...
  def test_valido
    un_turno_valido = Turn.new(turn_date: @fecha_hoy, hour: @fecha_hoy.strftime("%I:%M:%S"), reason_turn: "Renovacion de tarjeta", subsidiary: @una_sucursal, state: 'Pendiente', user_client: @un_cliente)
    assert un_turno_valido.valid?
  end

  # Falta el motivo (o razon) del turno que tiene validacion obligatoria
  def test_no_valido
    un_turno_no_valido = Turn.new(turn_date: @fecha_hoy, hour: @fecha_hoy.strftime("%I:%M:%S"), subsidiary: @una_sucursal, state: 'Pendiente', user_client: @un_cliente)
